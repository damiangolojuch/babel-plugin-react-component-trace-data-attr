...
export function TaggedTemplateExpression(path, state) {
  ...
  function insertBefore(node, id) {
    return t.callExpression(t.memberExpression(node, t.identifier('attrs')), [
      t.objectExpression([
        t.objectProperty(
          t.StringLiteral(options.styledComponentsAttribute),
...
export function CallExpression(path, state) {
  ...
  try {
    // simple case
    if (isStyledPrefix(path.node.callee)) {
      const id = getIdFrom(path.parentPath)
      if (!id) return
      path.node.callee = insertAfter(path.node.callee, id)
      return
    }
  ...
  function isStyledPrefix(node) {
    // handle two forms: styled.div and styled(Comp)
    return (
      (t.isMemberExpression(node.callee.object) && isStyledComponentsBinding(node.callee.object.object)) ||
      (t.isCallExpression(node.callee.object) && isStyledComponentsBinding(node.callee.object.callee))
    )
